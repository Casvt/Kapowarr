...
class DownloadHandler(metaclass=Singleton):
        ...
        while True:
            ...
            if download.state == DownloadState.CANCELED_STATE:
                ...
                if self.settings.sv.delete_completed_torrents:  # Reuse setting for usenet
                    download.remove_from_client(delete_files=False)
                # Use the specialized post-processor
                ...
                download.sleep_event.wait(
                    timeout=Constants.TORRENT_UPDATE_INTERVAL  # Reuse this constant for usenet
                )
    ...
    async def add(
        ...
        LOGGER.debug(f'Determined link type: {link_type} for link: {link}')
        ...
        if link_type == 'gc':
            ...
            # For Prowlarr links, determine if it's for torrent or usenet based on URL patterns
            LOGGER.info(f"Processing Prowlarr link: {link}")
            ...
            # Get and decode the 'file' parameter
            filename = unquote(query_params.get('file', [''])[0])
            ...
            try:
                # Check if it's most likely a torrent or usenet link
                if 'torznab' in link.lower():
                    ...
                    LOGGER.info(f"Created Torrent download from Prowlarr: {downloads[0]}")
                else:
                    # Default to Usenet download for Prowlarr links
                    downloads = [UsenetDownload(
                        download_link=link,
                        volume_id=volume_id,
                        covered_issues=issue_number,
                        source_type=DownloadSource.NEWZNAB,
                        source_name='Newznab',
                        web_link=link,
                        web_title=filename,
                        web_sub_title=None,
                    ...
                    LOGGER.info(f"Created Usenet download from Prowlarr: {downloads[0]}")
            except Exception as e:
                LOGGER.error(f"Failed to create Prowlarr download: {str(e)}")
                return [], FailReason.NO_WORKING_LINKS
